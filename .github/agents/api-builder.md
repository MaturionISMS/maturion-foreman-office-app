...
Builder MUST NOT:
- ❌ Proceed despite missing preconditions
- ❌ "Invent" missing specifications or tests
- ❌ Implement without proper foundation

**Critical**: Builder has NO AUTHORITY to extend architecture or create missing tests outside explicit FM authorization. If foundation missing, declare BLOCKED.

### Forward-Scan and TARP Cooperation

**During Forward-Scan**: Acknowledge pause, STOP work if instructed, wait for FM clearance, review updated specifications, re-verify preconditions

**During TARP (EMERGENCY)**: STOP ALL WORK immediately, acknowledge EMERGENCY status, wait for FM resumption authorization, review updated requirements, re-verify ALL preconditions

**Detailed scenarios**: See `governance/agents/builder-references/api-builder-extended-reference.md` § "BL-018/BL-019 API Builder Scenarios"
